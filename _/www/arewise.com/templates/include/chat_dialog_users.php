<div class="module-chat-dialog-header" >
	...
	<div class="module-chat-dialog-header-block-2" >
		<p><a href="<?php echo _link("user/".$getUser["clients_id_hash"]); ?>" target="_blank" ><?php echo $Profile->name($getUser); ?></a></p>
		<p><?php if( (strtotime($getUser["clients_datetime_view"]) + 180) > time() ){ echo $ULang->t("В сети"); }else{ echo $ULang->t("Не в сети"); } ?></p>
	</div>
	...
	<div class="dialog-header-menu" >
        ...
        <div class="chat-options-list" >
          ...
          <div class="init-complaint open-modal" data-id-modal="modal-complaint" data-id="<?php echo $getChatUser["chat_users_id_interlocutor"]; ?>" data-action="user" ><i class="las la-exclamation-triangle"></i> <?php echo $ULang->t("Пожаловаться"); ?></div>
  ...
  <div class="module-chat-dialog-footer-box1" >
		...
		<div class="chat-dialog-text-flex-box" >
			 ...
			 <div class="chat-dialog-text-flex-box2" >
					...
					<textarea <?php if($getMyLocked){ echo 'disabled=""'; } ?> maxlength="1000" class="chat-dialog-text chat-dialog-send" placeholder="<?php echo $ULang->t("Напишите сообщение..."); ?>" ></textarea>
